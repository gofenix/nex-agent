...
1. **Read the Feishu task custom field**: Use bash to call lark-cli and extract the "仓库" custom field value:
   ```
   bash("lark-cli task get <task_id> --json 2>/dev/null | grep -o '\"仓库\"[^,}]*' | grep -o '[^:\"]*$'")
   ```
...
2. Update task status: `bash("lark-cli task update <task_id> --status in_progress")`
3. Add a comment: `bash("lark-cli task comment <task_id> --content '排队中，前面的任务完成后自动执行。'")`
4. STOP — do not proceed further.

## Step 4: Execute opencode

Set task to in_progress (unless already set in Step 3):
```
bash("lark-cli task update <task_id> --status in_progress")
```

Run opencode with the task title and description as the prompt:
```
bash("cd <REPO_PATH> && opencode -m '<task_title>: <task_description>' 2>&1", timeout: 600)
```

Capture both exit code and output:
```
if cd "<REPO_PATH>" && opencode -m "<task_title>: <task_description>" 2>&1; then EXIT_CODE=$?; else EXIT_CODE=$?; fi
echo "EXIT_CODE=$EXIT_CODE"
```

Use the larger timeout (at least 600) since coding tasks can take a while. Check the EXIT_CODE from the output — 0 means success.

## Step 5: Report results

**If opencode succeeded (exit code 0):**
1. Update task status to completed: `bash("lark-cli task update <task_id> --status completed")`
2. Add a comment with output summary (first 500 characters, trim to avoid API limits):
   ```
   bash("lark-cli task comment <task_id> --content '执行完成。\n\n<output_summary>'")
   ```
...
2. Add a comment with the error output: `bash("lark-cli task comment <task_id> --content '执行失败。\n\n<error_output>'")`
