...
| 能力 | 说明 |
| --- | --- |
| **Self-evolving by design** | 通过统一的 `evolve` 在 memory、skill、tool、soul 和 code 各层演进 |
...
- `evolve` 可以通过 tool 层创建 workspace 级自定义 tool
...
通过 `evolve`，agent 仍然可以扩展 skill 和 tool，只是不再把这些低层入口作为主用进化接口。

### Code evolution

真正拉开差异的是源码级进化链路:

- `Reflect`: 内部反思阶段，输出结构化进化计划
- `Evolve`: 统一进化入口
- `Action`: 执行 soul、memory、skill、tool、code 各层变更
- `Action.Code`: 负责代码层的备份、校验、编译、加载、版本管理和回滚

这让 NexAgent 不只是“可配置的 agent”，而是一个具备真实自修改能力的 agent 系统。

```mermaid
flowchart LR
    A["Reflect<br/>规划正确层级"] --> B["Evolve<br/>路由变更"] --> C["Action<br/>执行 soul / memory / skill / tool / code"] --> D["Agent keeps running"]
```
